...
\CP[all]{Pourquoi mettre notre contrib  dans la section RW ?}
...
\CP{Problem de transition -- le paragraphe suivant ne devrait pas être dans cette section}
\CP{Fractal n'est pas introduit avant}
...
As introduced in the previous section \CP{laquelle? utilise un ref explicite, section x.y}, when automating distributed
...
has been introduced in the previous section \CP{redite+laquelle?} is its efficiency and in
...
from both $A$ and $B$. \CP{transition} First, Figure~\ref{fig:simh} displays
...
\begin{figure*}[t!]
  \CP{Réduide la taille des figures ??}
  \begin{center}
    \subfloat[SIMH]{
...
\CP{Et alors? pas de commentaire?}
...
\CP{K8S requires container to be started in any order, and thus any container must has a wait-procedure wrt his dependencies}
...
\CP{Idem, pas comprendre la boite colorée ci-après}
\begin{tcolorbox}[enhanced,attach boxed title to top left={yshift=-3mm,yshifttext=-1mm},
  colback=black!5!white,colframe=black!30,colbacktitle=black!60,
  title=Contribution,fonttitle=\bfseries,
  boxed title style={size=small,colframe=black!60,boxrule=0.2mm},
