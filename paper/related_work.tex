...
\item \emph{separation of concerns}: if the work (tool, framework or
  ...
  components while not needing to know their internal behaviors.
\end{itemize}
...
Figure~\ref{fig:parlevels} illustrates the four above parallelism levels
...
defined at the finer level of the tasks. In this case, commissioning
...
servers. Tasks are managed using only \textsc{SSH} and
...
software components.  Each role contains a set of files that each describe
...
some efforts have been done to formally define the \tosca
...
required database and thus will fail. For this reason, the metric
...
combination of metrics, \aeolus handles only partially the separation of
