\CP{Une intro à la section? est une description complète du modèle ou juste un aperçu ?}
...
at least one place that has been reached.
\CP{comment est definie une place qui a été 'reached' ?}
This binding relation is denoted $B_{S_p} \subseteq S_p \times \mathcal{P}(G)$. Intuitively,
...
state forever \CP{même si plus de jeton dans aucune place?}.
...
components as defined in Section~\ref{sec:mod:comp}, and their connections
...
types, \ie $(u,p) \in L$ only if $\types(u) = \types(p)$ \CP{Contrainte forte: il n'y a donc pas de ``sous-typage''/''héritage''}.
...
\CP{Créer une section pour la partie précédente, genre \mad configuration ?}
...
\paragraph{Structural constraints}{
  ...
  a simpler analysis tool can be used. \CP{Il faudrait expliciter
  plus clairement ce que ce papier fournit et ce qui est des perspectives/future work}
}
...
\paragraph{Consistency}{
  ...
  control on the tokens and their creation. \CP{La partie sur la reconf
  est à supprimer dans ce papier, non ? Juste à mentionner en perspective du papier.}
}
