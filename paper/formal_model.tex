...
%\SR{Add constraint that the initial place should be unique? Does not really matter for theoretical puroposes.}
...
\paragraph{Structural constraints}{
  ...
  A \mad component is said to be well-formed if there exists no place
  $\pi$ that depends on itself. Intuitively, the component, viewed as
  a directed graph, must be acyclic.
  ...
  \begin{proof}
    Since the sets of places, docks, transitions and actions are
    finite, the set of configurations is also finite. Therefore it is
    enough to show that there exists no cyclic execution $\langle \mk,
    \reached, \exec \rangle \semstep \dots \semstep \langle \mk,
    \reached, \exec \rangle$.
  ...
  \HC[Simon et Maverick]{c'est une très bonne idée mais j'aimerais que
  nous soumettions rapidement. Soit on laisse et on fait rapidement,
  soit on enlève}
  \begin{proof}
    ...
    Let $\reached$ denote the set of place that are eventually reached
    in that execution. By (i), the set $\Pi \setminus \reached$ is
    non-empty. Furthermore, because $c$ is well-formed, the
    precede-relation is well-founded, therefore there exists at least
    one place $\pi \in Pi \setminus \reached$ such that all places
    $\pi'$ that precede $\pi$ are in $\reached$. By (ii) and
    Lemma~\ref{thm:reachedplaces}, $\pi \in \reached$, leading to a
    contradiction.
  \end{proof}
