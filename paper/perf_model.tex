...
\property{If $v^\text{sink}$ is reachable from $v^\text{source}$ and $G$ has no
...
\property{If a place \emph{pTarget} is reachable from place \emph{pStart} (distinct
...
\property{In a \mad dependency graph, if there is at least one component in the
...
\property{If a component is well-formed then its corresponding dependency graph has
no cycle.}
...
\property{In an assembly with no deadlocks, if each port is bound to at most one
