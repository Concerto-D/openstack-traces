...
the relative difference between transitions. \CP{Cette phrase est un peu boiteuse car
...
longest \CP{== en nombre d'arc ? N'est pas plutot le plus lourd/grand? heaviest?}
...
to the action.
%
Figure~\ref{fig:transition_graph} depicts the dependency graph corresponding to an example
made of three transitions \emph{t1}, \emph{t2} and \emph{t3}.
\CP{transition? $E_\Theta$ non présenté :) iden pour les autres $E_X$}
\[
...
longest path \CP{idem: longueur == nb d'arc ou somme des poids? (je connais la réponse ;>)}
from $v^\text{source}$ to $v^\text{sink}$ in
