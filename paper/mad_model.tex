...
\mad is a formal component model inpired by \aeolus that enhances the automation, the separation of concerns and the parallelism level of distributed software commissionings. \mad is a procedural-oriented \emph{software configuration tools} and a component-based coordination model for commissioning procedures. Usually, component models are used to write distributed software in a modular fashion while having guarantees on its composition, thus improving separation of concerns between developers, easing the reusability of existing pieces of code and enhancing flexibility and maintainability of code. \mad makes use of these properties for the design of distributed software commissionings instead of code design.
...
\mad control component is a type containing an \net, \CP{Dire que cela est inspiré des réseaux de Pétri ?}\ie places that
...
\begin{figure}[tbp]
  \CP{L'exemple contient trop/pas assez d'information à ce stade du papier}
  \begin{center}
...
\paragraph{Example}{ Figure~\ref{fig:example} depicts a \mad
  ...
  service and data. \CP{redite} Figure~\ref{fig:example} shows the assembly of one
  ...
  component \CP{IP peu visible en Fig 2 -- comment on les configure?}, and the testing phase for Apache, called \texttt{check},
  uses the MariaDB service.} \CP{Ne faut il pas dire quelque part que les ports ne passent que les info de synchro
  et pas les données/services eux-même ? Par exemple est-ce \mad qui transfére les données comme l'@IP mdb?}
...
\CP{Mentionner qu'il est straightforward de definir un ADL ??}
