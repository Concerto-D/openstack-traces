...
In this section an evaluation of \mad on a real
case-study is presented, the commissioning of OpenStack. As in the previous
section, all results presented below can be reproduced by following
the lab on the following publicly available repository:
...
\HC[Helene]{Aller plus loin et discuter la sep of conc pour chacun
  ansible, aeolus et mad}
...
compared to \ansass, our reference, is displayed below the bars' edges.
...
\SR{``hardware configuration'' est un peu ambigu au vu de l'usage du mot
configuration ailleurs dans le papier. Je suggère ``superior hardware
capabilities'', or parler plus précisément de la capacité d'exécution parallèle.}
...
configuration of the cluster, the better the performance. Our claim
is that the physical nodes on which complex distributed
software systems are installed often have a very good hardware configuration and
