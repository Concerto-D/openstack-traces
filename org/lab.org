...
First, you need to recover the code on the [[https://gitlab.inria.fr/VeRDi-project/madeus-openstack-benchmarks][Gitlab]] repository.
#+BEGIN_SRC sh
git clone https://gitlab.inria.fr/VeRDi-project/madeus-openstack-benchmarks.git
#+END_SRC

Once you have that you can set up a virtual environment:
#+BEGIN_SRC sh
...
Then you need to copy the directory result from the paper that you wish to reproduce. 
...
*** Plotting the curves
Using the following command by replacing it with the result directory allows to reproduce the graphs from our paper.
#+BEGIN_SRC sh
python mad-enoslib.py analyze -b result_directory
#+END_SRC
...
The requirements are the [[https://click.palletsprojects.com/en/7.x/][click]] library, the [[https://plot.ly/python/][plotly]] library for the graph plotting and [[https://github.com/plotly/orca][orca]] to create the svgs from the plots.
*** Plotting the curves
To plot our graphs, we used the resulting files that are in the =results/ssh_bench/raw= directory.

They are sorted by type of experiment, parallel, sequential, dry-run or with the ssh connections.

To plot the graphs, we use the =graph_results.py= file is in the =results/ssh_bench= directory of the journal's repository. 
The command =analyze= is used to create the graphs, and the options to give are the directory containing the dryrun results
=-drp [directory]=, the directory containing the ssh results =sshp [directory]= and the type of experiment, whether
sequential or parallel with =-exp [parallel|sequential]=

#+BEGIN_SRC sh
python3 graph_results.py analyze -drp raw/seq_no_ssh -sshp raw/seq_ssh -exp sequential
#+END_SRC

#+NAME: example of a plotted curve for parallel assembly
[[./par_component.png]]
